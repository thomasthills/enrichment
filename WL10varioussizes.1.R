...
wordsInWorld=c(100, 500, 1000, 5000)
...
for(alphai in 1:length(alphas)){
  ...
  for(bis in 1:length(Betas)){
    ...
    for(woi in 1:Worlds){
      print(paste( "alpha = ", alphai[alphai], "; Beta = ", Betas[bis], "; world size= ", wordsInWorld[bis], "; simulation = ", woi, sep=""))  
      # Build the world
...
pdf(file="EntropySimvariablesizes.1.pdf", width=9, height=6)
par(mfrow=c(1,2))
...
legend(2.5, 60, legend=c(TeX('100'), TeX('500'), TeX('1000'),TeX('5000')), title=TeX('Environment size'), lty = 1:4, bty="n", lwd=1.4, cex = .9)
dev.off()
