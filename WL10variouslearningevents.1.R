...
wordsInWorld= rep(500, 5)
Associates = 2000 # Edges in environment
# Set learning events and age classes
learningEvents <- c(100, 500, 1000,2000)
ageEpochs = 4
Worlds = 10
Betas = rep(.1,4)


alphas <- 1 # this is 'a' for environment type
...
for(alphai in 1:length(alphas)){
  ...
  for(bis in 1:length(Betas)){
    ...
    for(woi in 1:Worlds){
      print(paste( "alpha = ", alphai[alphai], "; Beta = ", Betas[bis], "; world size= ", wordsInWorld[bis], "; simulation = ", woi, "; learn events = ", learningEvents[bis], sep=""))  
      # Build the world
      x <- 1:wordsInWorld[bis]
      ...
      difamt = wordsInWorld[bis]-length(V(ii))
      if (difamt > 0){
...
pdf(file="EntropySimvariousLearningEvents.1.pdf", width=9, height=6)
par(mfrow=c(1,2))
plot(1:4, Elist[[1]][1,], ylim = c(0, 2.5), cex = 0, xlab = "Epoch", ylab = "Entropy", cex.lab=1.5, xaxt="n")
...
plot(1:4, Slist[[1]][1,], cex = 0, ylim =c(0, 60), xlab = "Epoch", ylab = "Similarity", cex.lab = 1.5, xaxt="n")
axis(1, at=1:4, labels=c("1","2","3","4") )
...
legend(2.7, 60, legend=c(TeX('100'), TeX('500'), TeX('1000'),TeX('2000')), title=TeX('Learning events'), lty = 1:4, bty="n", lwd=1.4, cex = .9)
dev.off()
