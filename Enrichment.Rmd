...
```{r fig2, eval = TRUE, fig.cap="The growing mental lexicon. ", cache=TRUE}
...
for(lage in 1:ageEpochs){
 ...
 # make undirected graph from representation 
  ...
  if(lage == 1){
    text(0, 1.5, TeX("Learned lexicon: \\alpha = 1"))
  }
...
for(lage in 1:ageEpochs){
 ...
 # make undirected graph from representation 
  ...
  # label first one in 'learned'
  if(lage == 1){
    text(0, 1.5, TeX("Learned lexicon: \\alpha = 0"))
  }
  # label all with iterations
  its <- lage*learningEvents
  text(0, -1.5, paste("t =",its))
...
```{r entropySim, fig.cap="The rising entropy and falling similarity of the aging lexicon as a function of learning. ",fig.height=3.4, eval = FALSE, cache = TRUE}
...
```{r, echo=FALSE,out.width="100%", fig.cap="Entropy and similarity measures computed from the cognitive representation at different epochs of development.",fig.show='hold',fig.align='center'}
knitr::include_graphics(c("EntropySim.pdf"))
``` 
