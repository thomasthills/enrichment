...
ageEpochs = 4
Worlds =  100
Betas = seq(.1, 1.0, .3) # with context cue
...
for(alphai in 1:length(alphas)){
  ...
  for(bis in 1:length(Betas)){
    ...
    for(woi in 1:Worlds){
      ...
      n = length(V(iis)) +1 # number of cues (words) + context cue # remove context
      ...
      for(lage in 1:ageEpochs){
        ...
        # gle <- igraph::delete_vertices(gle, n) # not needed
...
#pdf(file="EntropySim1005002000200.01to1.pdf", width=9, height=6)
par(mfrow=c(1,2))
...
legend(1, 50, legend=c( TeX('.1'), TeX('.4'), TeX('.7'),TeX('1.0')), title=TeX('\\beta'), lty = 1:4, bty="n", lwd=1.4, cex = .9)
#dev.off()
