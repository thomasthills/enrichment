...
Associates = c(100, 1000, 10000) # Edges in environment
...
for(alphai in 1:length(alphas)){
  ...
  for(bis in 1:length(Betas)){
    ...
    for(woi in 1:Worlds){
      print(paste( "alpha = ", alphai[alphai], "; Beta = ", Betas[bis], "; world size= ", wordsInWorld[bis], "; simulation = ", woi, "; associates = ", Associates[bis], sep=""))  
      # Build the world
      x <- 1:wordsInWorld[bis]
      a = alphas[alphai] # set to 1 for ranking and 0 for ER with fixed number
      pairs <- c()
      for(i in 1:Associates[bis]){
...
pdf(file="EntropySimvariousassociations.1.pdf", width=9, height=6)
par(mfrow=c(1,2))
plot(1:4, Elist[[1]][1,], ylim = c(0, 2), cex = 0, xlab = "Epoch", ylab = "Entropy", cex.lab=1.5, xaxt="n")
...
plot(1:4, Slist[[1]][1,], cex = 0, ylim =c(0, 30), xlab = "Epoch", ylab = "Similarity", cex.lab = 1.5, xaxt="n")
axis(1, at=1:4, labels=c("1","2","3","4") )
...
legend(2.5, 30, legend=c(TeX('100'), TeX('1000'), TeX('10000') ), title=TeX('Environment pairs'), lty = 1:3, bty="n", lwd=1.3, cex = .9)
dev.off()
